...
#if defined(NDEBUG)
# error "RUPEE cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfb6a19a31293de12035e5cdaadce2a8552d3feeec6fa5f910101d9d8c579b566");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RUPEE: starting difficulty is 1 / 2^12
...
{
    // RUPEE: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // RUPEE: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // RUPEE: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1485789905, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Welcome to RUPEE Digital Currency";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f0a4288a56f217e0f5d5fe6e0cc324972b26c4863618cd42270bfae20c070a5ff461bac4c0ca1c72433186099c51b31dc62cd399d1b7c10a545025545738a9ed") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x85afdd95433b14b54a8455915a0c0a8aafb2c2da99635fc668cee8b7d040fd1e"));
...
unsigned char pchMessageStart[4] = { 0x3b, 0x69, 0xde, 0xd4 }; // RUPEE: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RUPEEMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RUPEEMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RUPEEMiner(CWallet *pwallet)
{
    printf("RUPEEMiner started\n");
    ...
    try { loop {
        ...
        printf("Running RUPEEMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RUPEEMiner, pwallet));
}
